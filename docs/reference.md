...
| `--E-timeout <dur>` | per-engine time-box, e.g. `8m` (0/empty = none). Bounds an engine that polls a remote service so it can't hang the run — notably **`--sca-timeout`**, since the Cx1 SCA results/report **export can stall** (see [sca-resolver.md](sca-resolver.md#troubleshooting-sca-export-is-slow-or-times-out)). A stall is reported as an **engine failure**, never a false breach. |
